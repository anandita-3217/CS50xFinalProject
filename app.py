...
from helpers import apology, login_required, format_date
# Init App
app = Flask(__name__)
...
def register():
    ...
    if request.method == "POST":
        ...
        if not username:
            ...
            return apology("Please Provide Confirmation", 400)
        elif password != confirmation:
            return apology("Passwords Do Not Match", 400)
        try:
            result = db.execute("INSERT INTO users (username, hash) VALUES(?, ?)",
                                username, generate_password_hash(password))
            session["user_id"] = result
            flash(f"Welcome to Exsplitter, {username}!")
            return redirect("/")
        except ValueError:
            return apology("Username Unavailable", 400)
...
@app.route("/")
def index():
    """Routes to landing page if no user is logged in else, takes to user dashboard"""
...
def create_group():
    if request.method == "POST":
        ...
        if not name:
            return apology("Please provide name", 400)
        elif not theme:
            return apology("Please provide theme", 403)
        group_id = db.execute("INSERT INTO groups(name, theme) VALUES (?, ?)", name, theme)
...
@login_required
def groups():
    """Shows all groups the logged in user is a part of"""
    groups = db.execute("""
            SELECT g.*
            FROM groups g
            JOIN group_members gm ON g.id = gm.group_id
...
def group(group_id):
    ...
    for expense in expenses:
        ...
        if not splits:
            ...
            ORDER BY u.username
            """, group_id)
            split_amount = expense['amount'] / len(members_list)
            splits = [{'username': m['username'], 'amount': split_amount} for m in members_list]
...
def add_member(group_id):
    ...
    if request.method == "POST":
        ...
        existing = db.execute("""
            ...
            """, group_id, user_id)
        if existing:
            return apology("User already in the group", 400)
        db.execute("""
            INSERT INTO group_members (group_id,user_id) VALUES(? ,?)
            """, group_id, user_id)
        flash(f"{username} added to the group!")
        ...
        return render_template("add_member.html", group_id=group_id, group=group, available_users=available_users)
...
def add_expense(group_id):
    ...
    if request.method == "POST":
        ...
        if split_type == "equal":
            ...
            for member in members:
                member_id = member["user_id"]
                custom_amount_key = f"split_{member_id}"
                custom_amount = request.form.get(custom_amount_key, "0")

                try:
                    custom_amount = float(custom_amount)
                    if custom_amount < 0:
                        return apology("Split amounts must be non-negative", 400)
                    custom_splits[member_id] = custom_amount
                    total_custom += custom_amount
                except ValueError:
                    return apology(f"Invalid custom amount for member {member_id}", 400)
            if abs(total_custom - amount) > 0.01:
                return apology(f"Custom splits (${total_custom:.2f}) must equal total amount (${amount:.2f})", 400)
            for member_id, split_amount in custom_splits.items():
                if split_amount > 0:
                    ...
                    if member_id != paid_by:
                        ...
                        """, expense_id, member_id, paid_by, split_amount)
                        debts_created += 1
        flash(f"Expense '{description}' added successfully!")
        return redirect(f"/group/{group_id}")
...
def delete_expense(expense_id):
    ...
    if not expense:
        return apology("You can only delete expenses you created", 403)
    db.execute("DELETE FROM expenses WHERE id = ?", expense_id)
    flash(f"Expense deleted!")
...
def balances():
    ...
    owes = db.execute("""
        SELECT d.id, d.amount, u.username as creditor,
               e.description, e.category, d.created_at,
               g.name as group_name,
               d.debtor_marked_paid, d.creditor_marked_paid
        FROM debts d
        JOIN users u ON d.creditor_id = u.id
        JOIN expenses e ON d.expense_id = e.id
        JOIN groups g ON e.group_id = g.id
    ...
    # Calculate days ago for each debt
    for debt in owes + owed:
        created = datetime.strptime(debt['created_at'], '%Y-%m-%d %H:%M:%S')
        days_ago = (datetime.now() - created).days
        debt['days_ago'] = days_ago
...
@app.route("/mark-paid-debtor/<int:debt_id>", methods=["POST"])
@login_required
def mark_paid_debtor(debt_id):
    """Debtor marks debt as paid (Step 1 of handshake)"""
    debt = db.execute("SELECT * FROM debts WHERE id = ? AND debtor_id = ?",
                      debt_id, session["user_id"])
    if not debt:
        return apology("Debt not found or you're not the debtor", 403)
    db.execute("""
        UPDATE debts
        SET debtor_marked_paid = 1, debtor_paid_at = CURRENT_TIMESTAMP
        WHERE id = ? """, debt_id)
    updated_debt = db.execute("SELECT * FROM debts WHERE id = ?", debt_id)[0]
    if updated_debt['creditor_marked_paid'] == 1:
        ...
        db.execute("""
            ...
            WHERE id = ?
        """, debt_id)
        flash("Payment fully confirmed by both parties! Debt settled!")
        return redirect("/balances?confetti=true")
    else:
        flash("You marked this as paid! Waiting for creditor confirmation...")
        return redirect("/balances")


@app.route("/mark-paid-creditor/<int:debt_id>", methods=["POST"])
@login_required
def mark_paid_creditor(debt_id):
    """Creditor confirms payment received (Step 2 of handshake)"""
    ...
    if not debt:
        return apology("Debt not found or you're not the creditor", 403)

    # Mark as confirmed by creditor
    db.execute("""
        UPDATE debts
        SET creditor_marked_paid = 1, creditor_paid_at = CURRENT_TIMESTAMP
        WHERE id = ?
    """, debt_id)
    updated_debt = db.execute("SELECT * FROM debts WHERE id = ?", debt_id)[0]

    if updated_debt['debtor_marked_paid'] == 1:
        db.execute("""
            UPDATE debts
            SET paid = 1, paid_at = CURRENT_TIMESTAMP
            WHERE id = ?
        ...
        flash("You confirmed receipt! Waiting for debtor to mark as paid...")
        return redirect("/balances")
